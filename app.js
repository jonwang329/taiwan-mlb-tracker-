...
function line(p,g){return window.TaiwanTodayStatLine(p,g?.stat||{});}
function scheduledLine(p,g){const order=Number(g?.battingOrder||0),spot=order?Math.ceil(order/100):0,position=g?.position||p.role||'';return p.group==='pitching'?'預定先發投手':`先發已確認${spot?` · 第 ${spot} 棒`:''}${position?` · ${position}`:''}`;}
function todayLine(p,g){return g?.scheduled&&!g?.onGame?scheduledLine(p,g):`${line(p,g)}${g?.live?' · LIVE':''}`;}
...
function detail(p,r){const n=names(p),today=Boolean(r.today),scheduled=today&&r.today.scheduled&&!r.today.onGame;return `<article class="player-detail" id="player-${p.id}"><header>${image(p,true)}<div><span class="level">${currentLevel(r)}</span><h3>${n.zh}</h3><b>${n.en}</b><p>${p.org} · ${p.role}</p></div></header><div class="today-detail"><span>今日賽事</span><strong>${today?(scheduled?'先發已確認':r.today.live?'LIVE · 已出賽':'已出賽'):'今日未出賽'}</strong><p>${today?todayLine(p,r.today):(p.group==='pitching'?'尚無下次登板資訊':'等待下一場比賽')}</p></div><section class="last-five"><div class="subhead"><h4>${p.group==='pitching'?'近 5 次登板':'近 5 場比賽'}</h4><span>LAST 5</span></div>${gameRows(p,r.games||[])}</section><section class="season-stats"><div class="subhead"><h4>本季成績</h4><span>SEASON</span></div>${metricGrid(p,r.season)}</section></article>`;}
  ...
  for(const teamId of ids){
    try{
      ...
      for(const g of ordered){
        if(!g.gamePk||seenGames.has(g.gamePk))continue;
        ...
        try{
          ...
          const battingOrder=Number(bp.battingOrder||0),isStarter=Boolean(bp.gameStatus?.isStarter||battingOrder);
          if(isStarter)return {date:gameTaiwanDate(g),level,stat:{},game:{gamePk:g.gamePk},live:false,scheduled:true,onGame:false,battingOrder,position:bp.position?.abbreviation||bp.position?.name||''};
...
function paint(results,statusText){const safeResults=players.map((_,i)=>results?.[i]||lastResults?.[i]||{levels:[],games:[],latest:null,today:null,season:{}}),summary=document.querySelector('#player-summary'),details=document.querySelector('#player-details');summary.innerHTML=summaryGroup('hitting',safeResults)+summaryGroup('pitching',safeResults);details.innerHTML=players.map((p,i)=>detail(p,safeResults[i])).join('');updateMetrics(safeResults);wireImages();document.querySelector('#last-update').textContent=statusText||`MLB API 已更新 · ${formatTime(lastSuccessAt||Date.now())}`;document.dispatchEvent(new CustomEvent('tracker:players-loaded',{detail:players}));}
async function refreshCentralSnapshot(){
  const response=await fetch(`data/dashboard-snapshot.js?_=${Date.now()}`,{cache:'no-store'});
  if(!response.ok)throw new Error(`中央快照 ${response.status}`);
  const text=await response.text(),match=text.match(/window\.CENTRAL_DASHBOARD_SNAPSHOT\s*=\s*(.*);\s*$/s);
  if(!match)throw new Error('中央快照格式錯誤');
  const snapshot=JSON.parse(match[1]);
  if(!Array.isArray(snapshot.players)||!Array.isArray(snapshot.results)||snapshot.players.length!==snapshot.results.length)throw new Error('中央快照不完整');
  setTrackedPlayers(snapshot.players);
  persistSnapshot(snapshot.results,Number(snapshot.savedAt)||Date.now());
  paint(lastResults,`中央資料已更新 · ${formatTime(lastSuccessAt)}`);
  return snapshot;
}
...
document.querySelector('#refresh-btn').addEventListener('click',async e=>{
  ...
  lastUpdate.textContent='正在更新最新資料…';
  try{
    await refreshCentralSnapshot();
  }catch(error){
    console.warn('Central snapshot refresh failed',error);
    lastUpdate.textContent=`保留最後有效資料 · ${formatTime(lastSuccessAt)}`;
  }finally{
    button.disabled=false;
  }
  if(window.TaiwanMlbUniverseScan)window.TaiwanMlbUniverseScan({force:true}).catch(()=>{});
