...
function scheduledLine(p,g){const order=Number(g?.battingOrder||0),spot=order?Math.ceil(order/100):0,position=g?.position||p.role||'';return p.group==='pitching'?'預定先發':`先發${spot?`｜${spot}棒`:''}${position?`｜${position}`:''}`;}
