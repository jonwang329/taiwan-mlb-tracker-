...
function identityHtml(player,level=player.role){return `<div class="player-top"><div><span class="team-badge">${player.org}</span><h3>${player.name}</h3><p>${level}</p></div><span class="status">${player.status}</span></div>`;}
function card(player,stats=loadingStats(),latest='正在連接 MLB/MiLB 資料…',games=[],insight=null){return `<article class="player-card">${photoHtml(player)}<div class="player-content">${identityHtml(player)}${insight?.label?`<span class="performance-tag">${insight.label}</span>`:''}<div class="game-label">TODAY'S GAME</div><div class="today-line">${latest}</div><div class="stats">${stats.map(([v,l])=>`<div class="stat"><b>${v}</b><span>${l}</span></div>`).join('')}</div>${insight?.summary?`<div class="placeholder"><b>INSIGHT</b><br>${insight.summary}</div>`:''}${recentGamesHtml(games)}</div></article>`;}
