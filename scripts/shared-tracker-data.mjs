...
async function playerSnapshot(player, reportDate, gameDate, fetcher) {
  ...
  const team = gameLogGame?.team?.name || latest?.team?.name || person.currentTeam?.name || player.org;
  ...
  const candidateTeamIds = [...new Set([gameLogGame?.team?.id, latest?.team?.id, ...games.slice(0,5).map(game => game.team?.id), person.currentTeam?.id].filter(Boolean))];
  const scheduledBatches = await Promise.all(candidateTeamIds.map(teamId => teamGames(teamId, gameDate, fetcher)));
  const scheduledGames = [...new Map(scheduledBatches.flat().filter(game => game?.gamePk).map(game => [game.gamePk, game])).values()];
