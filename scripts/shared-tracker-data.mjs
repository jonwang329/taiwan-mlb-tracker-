...
function performance(group, stat = {}) {
  return group === "pitching"
    ? `${stat.inningsPitched ?? "0"} IP, ${stat.strikeOuts ?? 0} K, ${stat.baseOnBalls ?? 0} BB, ${stat.earnedRuns ?? 0} ER`
    : `${stat.hits ?? 0}-for-${stat.atBats ?? 0}, ${stat.homeRuns ?? 0} HR, ${stat.rbi ?? 0} RBI, ${stat.stolenBases ?? 0} SB`;
}
...
async function playerSnapshot(player, reportDate, gameDate, fetcher) {
  ...
  const currentGameStatus = game ? await gameStatus(game, fetcher) : await teamGameStatus(person.currentTeam?.id, currentLevel.sportId, gameDate, fetcher);
...
export function comparable(snapshot) {
  return snapshot.players.map(({id,team,status,played,gameDate,gameStatus,performance,season,news,latestGameDate}) => ({id,team,status,played,gameDate,gameStatus,performance,season,news,latestGameDate}));
}
...
export function formatSummary(snapshot, period="final", test=false) {
  ...
  const sections = snapshot.players.map(player => [`\n【${player.name}｜${player.team}】`,`比賽：${player.gameStatus}`,`出賽：${player.played ? "有" : "無"}｜球員狀態：${player.status}`,`本場：${player.performance}${player.played ? `（${player.level}）` : ""}`,`球季：${player.season}`,`動態：${player.news}`,`觀察：${player.insight}`].join("\n"));
