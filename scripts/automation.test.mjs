...
test('watchlist UI updates Cloudflare directly without GitHub login or owner prompt',async()=>{
  ...
  assert.doesNotMatch(manager,/prompt\(/);
  assert.doesNotMatch(manager,/OWNER_STORAGE/);
});

test('Cloudflare Worker writes KV for trusted tracker-site mutations',async()=>{
  ...
  assert.match(worker,/TRUSTED_ORIGINS/);
  assert.match(worker,/https:\/\/jonwang329\.github\.io/);
  ...
  assert.match(worker,/mlbPlayer\(id\)/);
