...
const inputPath=process.env.WATCHLIST_INPUT || '/tmp/players.json';
const outputPath=process.env.WATCHLIST_OUTPUT || '/tmp/players-updated.json';
...
if(!Array.isArray(players)) throw new Error('Observation list is invalid');
...
if(action==='remove'){
  ...
  console.log(`Prepared removal of player ${playerId}`);
  process.exit(0);
}
...
await writeFile(outputPath,JSON.stringify(next,null,2)+'\n');
console.log(`Prepared add of ${player.name} (${player.id})`);
