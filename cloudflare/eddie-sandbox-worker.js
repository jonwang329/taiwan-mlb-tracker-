...
function studentPage(state, token) {
  ...
  const body = confirmed
    ? `<div class="confirmed">✓ 已確認<br><strong>${state.confirmed}</strong></div><p class="muted">另一個候選時段已自動釋放，不需要再通知教練。</p>`
...
async function handleDemoSend(request, env) {
  ...
  if (!nonce || nonce !== env.EDDIE_DEPLOY_NONCE) return json({ error: 'unauthorized' }, 401);
...
async function confirmStudent(request, env, token) {
  ...
  if (state.lineUserId) {
    await pushLineText(env, state.lineUserId, `✅ Eddie Training 已確認\n${state.confirmed}\n\n其他候選時段已自動釋放。`);
  }

  return json({ ok: true, state: { status: state.status, confirmed: state.confirmed, released: state.released } });
}

async function coachStatus(env) {
  const s = await env.EDDIE_KV.get(coachStateKey, 'json');
