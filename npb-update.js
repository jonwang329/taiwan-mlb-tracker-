(()=>{
  ...
  const koreaPlayers=[
    {name:'王彥程',team:'韓華鷹 Hanwha Eagles',role:'投手',level:'KBO 一軍 · No.19',stats:[['3.52','ERA'],['10-5','W-L'],['120⅓','IP'],['95','K']],today:'Last 5: 08/18 vs KIA · 5 IP · 3 ER · 3 K',note:'KBO 官方 2026 成績：23場、10勝5敗、WHIP 1.45、7 QS。現在是韓華鷹，不是 KIA Tigers；近期對 KIA 出賽可能容易混淆。',source:'https://eng.koreabaseball.com/Teams/PlayerInfoPitcher/Summary.aspx?pcode=56719'}
  ];
  ...
  root.innerHTML=`<section class="asia-country"><div class="npb-meta"><span class="npb-badge">JAPAN · NPB / FARM · ${japanPlayers.length} 位</span><span>官方 snapshot：${updated} · JST</span></div><div class="npb-list">${japanPlayers.map(card).join('')}</div></section><section class="asia-country korea-country"><div class="npb-meta"><span class="npb-badge korea-badge">KOREA · KBO · ${koreaPlayers.length} 位</span><span>官方 snapshot：${updated} · KST</span></div><div class="npb-list">${koreaPlayers.map(card).join('')}</div></section><p class="npb-disclaimer">Asia 頁把 Japan 與 Korea 放在同一個乾淨頁面，不再增加額外分頁。日本與韓國同為 UTC+9；兩邊各自使用 NPB / KBO 官方資料。</p>`;
