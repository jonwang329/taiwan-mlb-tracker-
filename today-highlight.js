(()=>{
  ...
  function hitter(player,result){
    const s=result?.today?.stat||{},hits=num(s.hits),hr=num(s.homeRuns),rbi=num(s.rbi),sb=num(s.stolenBases),ab=num(s.atBats),bb=num(s.baseOnBalls),k=num(s.strikeOuts),hbp=num(s.hitByPitch),pa=num(s.plateAppearances);
    const timesOnBase=hits+bb+hbp;
    const tags=[];let score=0,tone='positive';
    ...
    const hasStrongPositive=hr>0||hits>=2||timesOnBase>=3||rbi>=3||sb>=2;
    if(k>=3&&!hasStrongPositive){tags.unshift(`${k} K`);score=Math.max(score,48);tone='warning'}
    else if(k>=2&&!hasStrongPositive){tags.unshift(`${k} K`);score=Math.max(score,30);tone='warning'}
    if(ab>=4&&hits===0&&!hasStrongPositive){tags.push(`0-FOR-${ab}`);score=Math.max(score,34);tone='warning'}
    return tags.length?{player,score,tone,title:shortName(player),headline:[...new Set(tags)].slice(0,2).join(' · '),detail:`${hits}-for-${ab}${pa?` · ${pa} PA`:''}${bb?` · ${bb} BB`:''}${k?` · ${k} K`:''}${hbp?` · ${hbp} HBP`:''}${hr?` · ${hr} HR`:''}${rbi?` · ${rbi} RBI`:''}${sb?` · ${sb} SB`:''}`,href:`#player-${player.id}`}:null;
  }
  function pitcher(player,result){
    ...
    return tags.length?{player,score,tone:'positive',title:shortName(player),headline:[...new Set(tags)].slice(0,2).join(' · '),detail:`${s.inningsPitched??0} IP · ${h} H · ${er} ER · ${bb} BB · ${k} K`,href:`#player-${player.id}`}:null;
  }
  ...
  function pulse(snapshot){return snapshot.players.map((player,i)=>{const r=snapshot.results[i],s=r?.today?.stat||{};if(r?.today){if(player.group==='pitching')return `${shortName(player)} ${s.inningsPitched??0} IP · ${num(s.strikeOuts)} K · ${num(s.earnedRuns)} ER`;return `${shortName(player)} ${num(s.hits)}-${num(s.atBats)}${s.plateAppearances!=null?` · ${num(s.plateAppearances)} PA`:''}${num(s.baseOnBalls)?` · ${num(s.baseOnBalls)} BB`:''}${num(s.strikeOuts)?` · ${num(s.strikeOuts)} K`:''}${num(s.homeRuns)?` · ${num(s.homeRuns)} HR`:''}${num(s.rbi)?` · ${num(s.rbi)} RBI`:''}`;}const status=r?.currentStatus;const level=status?.level||r?.latest?.level||r?.levels?.find?.(x=>x?.season)?.level||'';return `${shortName(player)} ${level?`${level} · `:''}今日暫無出賽`;});}
  ...
  function render({preserveIndex=false}={}){const root=document.querySelector('#today-highlight');if(!root)return;const snapshot=readSnapshot();if(!snapshot){root.hidden=true;return;}const highlights=buildHighlights(snapshot),played=snapshot.results.filter(r=>r?.today).length;if(!preserveIndex&&highlights.length!==lastHeroCount)heroIndex=0;let hero;if(highlights.length)hero=highlights[heroIndex%highlights.length];else if(played)hero={tone:'neutral',title:'TODAY WATCH',headline:`今天已有 ${played} 位追蹤球員出賽`,detail:'目前沒有達到 Highlight 門檻的特殊表現，持續等待賽事更新。',href:'#summary-title'};else hero={tone:'neutral',title:'TODAY WATCH',headline:'等待今日旅美球員賽事',detail:'有重要表現時，這裡會自動顯示今日最值得注意的球員。',href:'#summary-title'};const tickerItems=pulse(snapshot);const heroCount=highlights.length;root.hidden=false;root.dataset.tone=hero.tone||'positive';root.innerHTML=`<a class="highlight-hero" href="${esc(hero.href)}"><span class="highlight-kicker">今日重點 · TODAY’S HIGHLIGHT${heroCount>1?` · ${heroIndex%heroCount+1}/${heroCount}`:''}</span><strong>${esc(hero.title)}</strong><h2>${esc(hero.headline)}</h2><p>${esc(hero.detail)}</p><i aria-hidden="true">›</i></a><div class="highlight-ticker" aria-label="所有追蹤球員今日動態"><span>LIVE PULSE</span><div class="highlight-ticker-window"><div class="highlight-ticker-track">${tickerItems.map(text=>`<b>${esc(text)}</b>`).join('<em>•</em>')}</div></div></div>`;if(!preserveIndex)armHeroRotation(heroCount);}
