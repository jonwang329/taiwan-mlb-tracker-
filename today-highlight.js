(()=>{
  ...
  function pitcher(player,result){const s=result?.today?.stat||{},innings=ip(s.inningsPitched),er=num(s.earnedRuns),k=num(s.strikeOuts),bb=num(s.baseOnBalls),h=num(s.hits);const tags=[];let score=0;if(innings>=7&&er<=2){tags.push('DOMINANT START');score+=85}else if(innings>=5&&er<=2){tags.push('QUALITY OUTING');score+=60}if(k>=10){tags.push(`${k} K`);score+=80}else if(k>=8){tags.push(`${k} K`);score+=55}else if(k>=6){tags.push(`${k} K`);score+=32}if(innings>=3&&er===0){tags.push('SCORELESS');score+=38}return tags.length?{player,score,title:shortName(player),headline:[...new Set(tags)].slice(0,2).join(' · '),detail:`${s.inningsPitched??0} IP · ${h} H · ${er} ER · ${bb} BB · ${k} K`,href:`#player-${player.id}`}:null;}
  ...
  function pulse(snapshot){return snapshot.players.map((player,i)=>{const r=snapshot.results[i],s=r?.today?.stat||{};if(r?.today){if(player.group==='pitching')return `${shortName(player)} ${s.inningsPitched??0} IP · ${num(s.strikeOuts)} K · ${num(s.earnedRuns)} ER`;return `${shortName(player)} ${num(s.hits)}-${num(s.atBats)}${s.plateAppearances!=null?` · ${num(s.plateAppearances)} PA`:''}${num(s.baseOnBalls)?` · ${num(s.baseOnBalls)} BB`:''}${num(s.homeRuns)?` · ${num(s.homeRuns)} HR`:''}${num(s.rbi)?` · ${num(s.rbi)} RBI`:''}`;}const level=r?.latest?.level||r?.levels?.find?.(x=>x?.season)?.level||'';return `${shortName(player)} ${level?`${level} · `:''}今日暫無出賽`;});}
  function render(){const root=document.querySelector('#today-highlight');if(!root)return;const snapshot=readSnapshot();if(!snapshot){root.hidden=true;return;}const highlights=buildHighlights(snapshot),played=snapshot.results.filter(r=>r?.today).length;let hero;if(highlights.length)hero=highlights[0];else if(played)hero={title:'TODAY WATCH',headline:`今天已有 ${played} 位追蹤球員出賽`,detail:'目前沒有達到 Highlight 門檻的特殊表現，持續等待賽事更新。',href:'#summary-title'};else hero={title:'TODAY WATCH',headline:'等待今日旅美球員賽事',detail:'有重要表現時，這裡會自動顯示今日最值得注意的球員。',href:'#summary-title'};const tickerItems=pulse(snapshot);root.hidden=false;root.innerHTML=`<a class="highlight-hero" href="${esc(hero.href)}"><span class="highlight-kicker">今日重點 · TODAY’S HIGHLIGHT</span><strong>${esc(hero.title)}</strong><h2>${esc(hero.headline)}</h2><p>${esc(hero.detail)}</p><i aria-hidden="true">›</i></a><div class="highlight-ticker" aria-label="所有追蹤球員今日動態"><span>LIVE PULSE</span><div class="highlight-ticker-window"><div class="highlight-ticker-track">${tickerItems.map(text=>`<b>${esc(text)}</b>`).join('<em>•</em>')}</div></div></div>`;}
