...
const OWNER_STORAGE='twmlb_owner_key';
...
const ownerFromUrl=qs.get('owner');
if(ownerFromUrl){
  localStorage.setItem(OWNER_STORAGE,ownerFromUrl);
  qs.delete('owner');
  const clean=`${location.pathname}${qs.toString()?`?${qs.toString()}`:''}${location.hash}`;
  history.replaceState(null,'',clean);
}
function ownerKey(){return localStorage.getItem(OWNER_STORAGE)||'';}
...
async function updateWatchlist(action,id){
  ...
  let key=ownerKey();
  if(!key){key=prompt('請輸入 Taiwan MLB Tracker owner key');if(!key)return;localStorage.setItem(OWNER_STORAGE,key);}
  ...
  try{
    ...
    if(response.status===401){localStorage.removeItem(OWNER_STORAGE);throw new Error('OWNER_KEY');}
    ...
    status.textContent=e.message==='OWNER_KEY'?'Owner key 不正確，請重新開啟你的管理連結':'更新失敗：'+e.message;
    console.error(e);
  }
}
