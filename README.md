...
## 2026-09-18 LINE / Website Today-state split-brain fix

- Root cause: LINE and the Website independently resolved today's MLB/MiLB appearance. Cloudflare LINE could see a newer boxscore while the browser/central snapshot still showed `today: null`.
- The observation Worker now exposes a read-only `/today` canonical feed generated by the same server-side Today resolver used by Cloudflare notification logic.
- The Website now reconciles its browser MLB result against that canonical Cloudflare Today feed by immutable MLB person ID. A confirmed canonical appearance cannot be erased by an older snapshot or stale browser state.
- `app.js` is cache-busted in `index.html` so mobile Safari does not keep the pre-fix dashboard logic.
- Fallback remains safe: if the Cloudflare Today endpoint is temporarily unavailable, the Website keeps its existing MLB/browser result rather than blanking the dashboard.
