...
- K% / BB% labels carry the percent sign; mobile values are displayed without a second `%` character to reduce visual clutter.
- K% and BB% each show restrained same-league benchmark context; interpretation is role-aware (pitcher K% higher is better, pitcher BB% lower is better; hitter K% lower is better, hitter BB% higher is better).
- League comparison uses compact scorecard notation: `↑ 8% · LG`, `↓ 6% · LG`, or `≈ LG`. The arrow always means better/worse performance versus the same-league environment, so lower-is-better pitching metrics remain intuitive.
...
- If a trustworthy benchmark is unavailable, the dashboard shows no league-context annotation rather than estimating one.
